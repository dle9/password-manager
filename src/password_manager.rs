...
impl PasswordManager {
    pub fn new(username: String, master_password: String, new_user: bool) -> Self {
        ...
        if new_user {File::create(&format!("users/{}.txt", username.clone())).expect("Failed to create file");}
    ...
    pub fn add_password(&mut self, service: String) {
        ...
        let mut file = OpenOptions::new()
            .append(true)
            .open(&format!("users/{}.txt", self.get_username()))
    ...
    pub fn get_password(&self, service: String) -> Option<String> {
        ...
        let file = File::open(&format!("users/{}.txt", self.get_username())).expect("Failed to open file");
